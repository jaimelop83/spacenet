...
def parse_args():
    parser = argparse.ArgumentParser(description="Download JWST preview images from MAST.")
...
def main():
    ...
    mask_preview = [str(pt).upper().startswith("PREVIEW") for pt in products["productType"]]
    products = products[mask_preview]
    if len(products) == 0:
        raise SystemExit("No preview products found.")

    mask_ext = [
        str(name).lower().endswith((".jpg", ".jpeg", ".png"))
        for name in products["productFilename"]
    ]
    products = products[mask_ext]
    if len(products) == 0:
        raise SystemExit("No preview PNG/JPG products found.")

    if len(products) > args.max_files:
        products = products[: args.max_files]

    if args.dry_run:
        print(f"Would download {len(products)} preview files to {out_dir}")
        return

    results = Observations.download_products(products, download_dir=str(out_dir))

    manifest_path = out_dir / "jwst_preview_manifest.csv"
    ...
    print(f"Downloaded {len(results)} previews to {out_dir}")
