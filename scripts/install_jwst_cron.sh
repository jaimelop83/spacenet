...
POST_LINE='20 1 * * * /home/jaimelop/spacenet/scripts/postprocess_fits_ood.sh >> /mnt/personal_drive/jwst/logs/jwst_post_$(date +\%F).log 2>&1'
CRON_TAG="# jwst_fits_download"
ROTATE_TAG="# jwst_fits_rotate"
CONVERT_TAG="# jwst_fits_png"
OOD_TAG="# jwst_fits_ood"
POST_TAG="# jwst_fits_post"

mkdir -p /mnt/personal_drive/jwst/logs

existing="$(crontab -l 2>/dev/null || true)"
existing="$(printf "%s\n" "$existing" | grep -v "$CRON_TAG" | grep -v "$ROTATE_TAG" | grep -v "$CONVERT_TAG" | grep -v "$OOD_TAG" | grep -v "$POST_TAG" || true)"
existing="${existing}\n${CRON_LINE} ${CRON_TAG}"
